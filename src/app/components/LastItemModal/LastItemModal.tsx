...
import styles from './lastItemModal.module.css';
import { closeCartModal, removeMovieFromCart } from '@/app/redux/features/cart';
...
export default function LastItemModal() {
    const dispatch = useDispatch();
    const movieId = useSelector((state: RootState) => state.cart.showCartModal.id);

    const handleModalClose = () => {
        dispatch(removeMovieFromCart(movieId));
    ...
    return createPortal(
        <div className={styles.modal} onClick={handleBackgroundClick}>
            <div onClick={(event) => event.stopPropagation()} className={styles.modalContainer}>
                <div className={styles.modalTitleContainer}>
                    <div className={`${styles.modalTitleText} ${robotoBold.className}`}>
                        Удаление билета
                    </div>
                    <div className={styles.modalCloseBtn} onClick={handleCloseBtnClick}>
                        <svg
                ...
                <div className={`${styles.modalText} ${robotoRegular.className}`}>
                    Вы уверены, что хотите удалить билет?
                </div>
                <div className={styles.modalButtonsContainer}>
                    <button
                        className={`${robotoRegular.className} ${styles.yes}`}
                        onClick={handleModalClose}
                    >
                        Да
                    </button>
                    <button
                        className={`${robotoRegular.className} ${styles.no}`}
