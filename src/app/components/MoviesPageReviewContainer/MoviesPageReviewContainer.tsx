import styles from './moviesPageReviewContainer.module.css';

import { Roboto } from 'next/font/google';

import { useGetMovieReviewsByIdQuery } from '@/app/redux/services/moviesApi';
...
export default function MoviesPageReviewContainer(props: { id: string }) {
    const { data, error, isLoading } = useGetMovieReviewsByIdQuery(props.id);
                    ...
                    <div key={review.id} className={styles.reviewContainer}>
                        ...
                        <div className={styles.reviewInfo}>
                            <div className={styles.reviewTitle}>
                                <div className={robotoBold.className}>{review.name}</div>
                                <div>
                                    <span className={robotoRegular.className}>Оценка: </span>
                                    <span className={robotoBold.className}>{review.rating}</span>
                                </div>
                            </div>
